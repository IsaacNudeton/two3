/*
 * test_binary_forward_parity.cu — GPU vs GPU-Workspace Forward Parity
...
int main(void) {
    printf("============================================\n");
    printf("  Binary Forward: Multi-Shape Parity\n");
    printf("============================================\n\n");
    ...
    for (int i = 0; i < n_shapes; i++) {
        ...
        float *h_output_gpu = (float*)malloc(sh.S * sh.M * sizeof(float));
        float *h_output_ws = (float*)malloc(sh.S * sh.M * sizeof(float));
        ...
        /* Original GPU forward */
        binary_project_batch_gpu(&gpu_weights, h_input, h_output_gpu, sh.S, sh.K);
        ...
        /* Workspace forward */
        binary_project_batch_gpu_ws(&gpu_weights, &ws, h_input, h_output_ws, sh.S, sh.K);
        
        /* Compare */
        float err = max_error(h_output_gpu, h_output_ws, sh.S * sh.M);
