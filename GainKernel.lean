...
theorem reservoir_contraction
  ...
  by_cases hRC : R = C
  · simp [hRC]
  · have hne : |R - C| ≠ 0 := abs_ne_zero.mpr (sub_ne_zero.mpr hRC)
    have hlt : 1 - γ < 1 := by linarith
    nlinarith [abs_nonneg (R - C), abs_pos.mpr (sub_ne_zero.mpr hRC)]
...
theorem cfl_bound_sufficient
  ...
  -- This is algebraic manipulation showing α < bound ⟹ CFL holds
  field_simp at *
  nlinarith [mul_pos hγ hC, mul_pos hγ hβ, sub_pos.mpr h_phys]
